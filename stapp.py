...
def extract_characters_from_text(text, client):
    ...
    for line in response.content.strip().split("\n"):
        if ":" in line:
            name, description = line.split(":", 1)
            characters[name.strip("1234567890. *")] = description.strip()  # Clean up numbering and bolding
    return characters
...
if not api_key:
    ...
    if "emotional_model" not in st.session_state:
        st.session_state.emotional_model = EmotionalModel()
    if "user_id" not in st.session_state:
        st.session_state.user_id = "Anonymous"  # Default value

    # User ID input (persists across interactions)
    ...
    if st.session_state.dialogue_styles and character:
        if prompt := st.chat_input("Enter your topic or question:"):
            ...
            with st.chat_message("user"):
                st.markdown(prompt)
            save_message(st.session_state.user_id, character, "user", prompt, 0, 0, 0)

            # Calculate emotional states
            anger = st.session_state.emotional_model.calculate_anger()
            sadness = st.session_state.emotional_model.calculate_sadness()
            joy = st.session_state.emotional_model.calculate_joy()

            # Retrieve recent conversation history for this user and character
            history = get_user_history(st.session_state.user_id)
            recent_history = [h for h in history if h["character"] == character][-3:]  # Last 3 exchanges
            ...
            # Check for explicit "Do you know" query
            search_term = None
            if "do you know" in prompt.lower():
                search_term = prompt.lower().split("do you know")[-1].strip().rstrip("?")
                past_interactions = search_user_history(search_term)
            ...
            save_message(st.session_state.user_id, character, "assistant", ai_response, anger, sadness, joy)
    else:
        st.info("Upload a fictional text to start chatting with its characters.")